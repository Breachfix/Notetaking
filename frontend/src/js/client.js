  ...
  note: {
    ...
    delete(noteId, isNoteExists) {
      ...
      if ($note) {
        ...
        // If the note doesn't exist in the UI, log a warning
        console.warn(`Note with ID "${noteId}" not found in the UI. It may have been removed already.`);
      }
    
      // If there are no more notes, show the empty notes template
      if (!isNoteExists) {
        $notePanel.innerHTML = emptyNotesTemplate;
      }
    }
